...
- If `go` is available, the installer should build the NeuronFS CLI.
- If `go` is missing, the installer should fail clearly unless hook-only mode was explicitly allowed.
