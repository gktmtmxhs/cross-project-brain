...
if (!targetFile) {
  process.stderr.write("Usage: node scripts/cpb-patch-neuronfs-hook.mjs <path-to-v4-hook.cjs>\n");
  process.exit(1);
}
...
const helperRequireBlock =
  ...
  "const REPO_ROOT = process.env.CPB_REPO_ROOT || path.resolve(__dirname, '..', '..', '..');\n" +
  "let selectiveInjection = null;\n" +
  "try {\n" +
  "    selectiveInjection = require(path.join(REPO_ROOT, 'scripts', 'cpb-selective-injection.cjs'));\n" +
  "} catch (_) {}\n";
