...
const neuronfsBinary = path.join(neuronfsInstallDir, "neuronfs");
const rebuildScript = path.join(repoRoot, "scripts", "cpb-rebuild-runtime-brain.sh");
const autogrowthLogPath = path.join(agentRoot, "logs", "personal-autogrowth-corrections.jsonl");
const pollMs = Number.parseInt(
  process.env.CPB_AUTOGROWTH_POLL_MS ?? process.env.MUINONE_NEURONFS_AUTOGROWTH_POLL_MS ?? "2000",
  10,
);
...
function isPraiseEntry(entry, neuronPath) {
  if (entry.type === "correction" && entry.text === "PD칭찬") {
    return true;
  }
  const praiseRegex = /칭찬|잘\s*쓰셨습니다|좋아|훌륭|완벽|최고/i;
  return praiseRegex.test(String(entry.text ?? "")) || neuronPath.toLowerCase().includes("dopamine");
}

function isAuthorizedPraise(entry) {
  const authorId = String(entry.author || entry.source || "").toLowerCase();
  return authorId === "pm" || authorId === "basement_admin" || authorId.includes("pd");
}
...
function writeLearningMemory(targetBrain, neuronPath, entry) {
  ...
  if (typeof entry.scope === "string" && entry.scope.trim() !== "") {
    lines.push(`scope: ${entry.scope.trim()}`);
  }
  if (typeof entry.audience === "string" && entry.audience.trim() !== "") {
    lines.push(`audience: ${entry.audience.trim()}`);
  }
  if (typeof entry.language === "string" && entry.language.trim() !== "") {
    lines.push(`language: ${entry.language.trim()}`);
  }
  if (typeof entry.role === "string" && entry.role.trim() !== "") {
    lines.push(`role: ${entry.role.trim()}`);
  }
  if (typeof entry.career_track === "string" && entry.career_track.trim() !== "") {
    lines.push(`career_track: ${entry.career_track.trim()}`);
  }
  if (typeof entry.surface === "string" && entry.surface.trim() !== "") {
    lines.push(`surface: ${entry.surface.trim()}`);
  }
  if (typeof entry.env === "string" && entry.env.trim() !== "") {
    lines.push(`env: ${entry.env.trim()}`);
  }
  if (typeof entry.skill === "string" && entry.skill.trim() !== "") {
    lines.push(`skill: ${entry.skill.trim()}`);
  }
  if (typeof entry.problem === "string" && entry.problem.trim() !== "") {
    lines.push(`problem: ${entry.problem.trim()}`);
  }
  if (typeof entry.encountered_at === "string" && entry.encountered_at.trim() !== "") {
    lines.push(`encountered_at: ${entry.encountered_at.trim()}`);
  }
  if (typeof entry.resolved_at === "string" && entry.resolved_at.trim() !== "") {
    lines.push(`resolved_at: ${entry.resolved_at.trim()}`);
  }
  if (typeof entry.root_cause === "string" && entry.root_cause.trim() !== "") {
    lines.push(`root_cause: ${entry.root_cause.trim()}`);
  }
  if (typeof entry.fix === "string" && entry.fix.trim() !== "") {
    lines.push(`fix: ${entry.fix.trim()}`);
  }
  ...
  if (typeof entry.source === "string" && entry.source.trim() !== "") {
    lines.push(`source: ${entry.source.trim()}`);
  }
...
function applyEntry(entry, rawLine) {
  ...
  if (isPraiseEntry(entry, neuronPath)) {
    if (!isAuthorizedPraise(entry)) {
      log(`skipped unauthorized praise for ${neuronPath}`);
      return false;
    }
    const output = runNeuronfs(targetBrain, ["--signal", "dopamine", neuronPath]);
    neuronPath = parseResolvedPath(output, neuronPath);
    writeLearningMemory(targetBrain, neuronPath, entry);
    log(`${scope} dopamine -> ${neuronPath}`);
    return true;
  }
...
function rebuildRuntime() {
  ...
  log("runtime rebuilt from updated global/project brain");
}
...
function acquireProcessingFile() {
  ...
  if (!fs.existsSync(runtimeCorrectionsPath)) {
    return "";
  }
...
async function main() {
  ...
  log(`operator autogrowth watching ${runtimeCorrectionsPath}`);
  while (keepRunning) {
    try {
      syncCorrectionsOnce();
    } catch (error) {
  ...
  log("operator autogrowth stopped");
}
